...
#include <vector>
std::vector<std::pair<double, double>> trafficData; // stores time vs traffic rate (Mbps)
...
/* void LogTrafficRate(Ptr<PacketSink> sink, double interval)
{
    static double lastTotalRx = 0; // Store last received bytes
    double currentTotalRx = sink->GetTotalRx(); // Get current received bytes
    double trafficRate = (currentTotalRx - lastTotalRx) * 8 / (interval * 1e6); // Convert to Mbps
    lastTotalRx = currentTotalRx; // Update for the next call

    // Get current simulation time
    double currentTime = Simulator::Now().GetSeconds();

    // Store the traffic rate for plotting later
    trafficData.push_back(std::make_pair(currentTime, trafficRate));

    // Schedule next call to log traffic rate
    Simulator::Schedule(Seconds(interval), &LogTrafficRate, sink, interval); 
} 
*/


void SetupVbrTraffic(Ptr<Node> ueNode, Ptr<Node> remoteHost, Ipv4Address remoteHostAddr, Time simTime, double dataRate, uint32_t packetSize, uint16_t port)
{

    // uint16_t port = 50000;
    OnOffHelper onOff("ns3::UdpSocketFactory", InetSocketAddress(remoteHostAddr, port));

    // Set the data rate and packet size
    onOff.SetConstantRate(DataRate(std::to_string(dataRate) + "Mbps"), packetSize);

    // Setting up burst characteristics for VBR with random variation
    onOff.SetAttribute("OnTime", StringValue("ns3::ExponentialRandomVariable[Mean=2]")); // On time varies with a mean of 2 seconds
    onOff.SetAttribute("OffTime", StringValue("ns3::UniformRandomVariable[Min=0.5|Max=5]")); // Off time varies between 0.5 and 5 seconds

    // Install the application on the UE
    ApplicationContainer apps = onOff.Install(ueNode);
    apps.Start(Seconds(0.0)); // Start at the beginning of the simulation
    apps.Stop(simTime); // Stop at the simulation time (variable)

    // Setup a packet sink on the remote host to receive the traffic
    PacketSinkHelper sink("ns3::UdpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), port));
    ApplicationContainer sinkApps = sink.Install(remoteHost);
    sinkApps.Start(Seconds(0.0)); // Start at the beginning of the simulation
    sinkApps.Stop(simTime); // Stop at the simulation time (variable)
}




/* void PrintTrafficDataToCSV()
{
    std::ofstream outFile("traffic_data.csv");
    outFile << "Time,TrafficRate(Mbps)" << std::endl;
    for (const auto& data : trafficData)
    {
        outFile << data.first << "," << data.second << std::endl;
    }
    outFile.close();
    std::cout << "Traffic data saved to traffic_data.csv" << std::endl;
}
*/
...
main(int argc, char* argv[])
{
    ...
    // Declare variables to store input parameters
    double dataRateMbps = 10.0;  // Data rate in Mbps
    Time simTime = Seconds(1);   // Simulation time in seconds
    uint32_t packetSize = 60;  // Packet size in bytes
    uint32_t numberOfUes = 1;

    // Convert dataRate to string to pass to SetupVbrTraffic
    // std::string dataRateStr = std::to_string(dataRate) + "Mbps";
    /*
    ...
    cmd.AddValue("dataRate", "The data rate for the traffic in Mbps", dataRateMbps);
    cmd.AddValue("simTime", "The simulation time", simTime);
    cmd.AddValue("packetSize", "The packet size", packetSize);
    cmd.AddValue("numberOfUes", "The number of UEs in the simulation", numberOfUes);
    ...
    // Mobility for gNB (assuming a fixed position)
    MobilityHelper mobility;
    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    Ptr<ListPositionAllocator> positionAllocGnb = CreateObject<ListPositionAllocator>();
    positionAllocGnb->Add(Vector(0.0, 0.0, 10.0));  // Position the gNB
    mobility.SetPositionAllocator(positionAllocGnb);
    mobility.Install(gnbContainer);

    // Mobility for UEs (ensuring different positions)
    Ptr<ListPositionAllocator> positionAllocUe = CreateObject<ListPositionAllocator>();
    for (uint32_t i = 0; i < ueContainer.GetN(); ++i)
    {
    // Position UEs with distinct positions. Increase the distance for each UE.
    positionAllocUe->Add(Vector(gnbUeDistance + 10 * i, 0.0, 1.5));  // Spread UEs out in space
    }
    ...
    for (uint32_t i = 0; i < ueContainer.GetN(); ++i)
    {
    Ptr<Node> ueNode = ueContainer.Get(i);  // Get the current UE node
    Ipv4Address ueAddress = ueIpIface.GetAddress(i);  // Get the IP address of the current UE
    uint16_t port = 50000 + i;
    // Set up VBR traffic for each UE
    SetupVbrTraffic(ueNode, remoteHost, ueAddress, simTime, dataRateMbps, packetSize, port);
    }
    ...
    */
    ...
    Simulator::Destroy();
    // PrintTrafficDataToCSV();
    return 0;
}


// ./ns3 run scratch/cttc-nr-mimo-demo-vbr.cc